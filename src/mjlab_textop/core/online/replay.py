...
def load_sliced_mjlab_npz_blocks(
    ...
    *,
    ...
    motion = load_mjlab_motion(path)
    if frames > motion.num_frames:
        raise ValueError(
            f"Requested {frames} frames from {path}, "
            f"but motion only has {motion.num_frames}"
        )

    joint_pos_textop = reindex_mjlab_g1_joints_to_textop(motion.joint_pos[:frames])
    joint_vel_textop = reindex_mjlab_g1_joints_to_textop(motion.joint_vel[:frames])
    anchor_pos_w = motion.body_pos_w[:frames, TEXTOP_ROOT_BODY_INDEX]
    anchor_quat_w = motion.body_quat_w[:frames, TEXTOP_ROOT_BODY_INDEX]

    blocks = []
    for start in range(0, frames, block_size):
        stop = min(start + block_size, frames)
        blocks.append(
            TextOpMotionBlock(
