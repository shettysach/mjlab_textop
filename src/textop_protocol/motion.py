...
def validate_motion_block(block: MotionBlock) -> MotionBlock:
    ...
    if block.control.request_id is not None and (
        not isinstance(block.control.request_id, int)
        or isinstance(block.control.request_id, bool)
        or block.control.request_id < 0
    ):
        raise ValueError("Block request_id must be a non-negative integer or None")
    for name, value in (
        ("joint_vel", joint_vel),
