...
def test_green_square_stop_env_cfg_has_fixed_goal_eval_terms() -> None:
    ...
    assert (
        ...
        == GREEN_SQUARE_STOP_TASK_CFG.stop_trigger_radius
    )
...
def test_green_square_marker_spec_fn_adds_visual_non_colliding_geom() -> None:
    ...
    assert set(walls) == {
        ...
        "green_square_back_wall",
    }
    assert tuple(walls["green_square_left_wall"].pos) == (15.5, 9.0, 0.75)
    assert tuple(walls["green_square_right_wall"].pos) == (15.5, -9.0, 0.75)
    assert tuple(walls["green_square_end_wall"].pos) == (33.0, 0.0, 0.75)
    assert tuple(walls["green_square_back_wall"].pos) == (-2.0, 0.0, 0.75)
...
def test_green_square_mdp_terms_use_true_goal_position() -> None:
    ...
    assert torch.allclose(
        mdp.robot_goal_distance(env, GREEN_SQUARE_STOP_TASK_CFG.goal_pos_w),
        torch.tensor([0.0]),
    )
    assert torch.allclose(mdp.robot_xy_speed(env), torch.tensor([0.05]))
    assert mdp.inside_goal_radius(
        env,
        GREEN_SQUARE_STOP_TASK_CFG.goal_pos_w,
        radius=GREEN_SQUARE_STOP_TASK_CFG.success_radius,
    ).tolist()
...
def test_green_square_success_held_requires_hold_time() -> None:
    ...
    cfg = SimpleNamespace(
        params={
            "goal_pos_w": GREEN_SQUARE_STOP_TASK_CFG.goal_pos_w,
            "success_radius": GREEN_SQUARE_STOP_TASK_CFG.success_radius,
            "speed_threshold": GREEN_SQUARE_STOP_TASK_CFG.speed_threshold,
            "hold_time_s": 0.15,
        }
    )
