...
def robot_goal_distance(
    ...
    return torch.linalg.norm(
        asset.data.root_link_pos_w[:, :2] - goal_pos_tensor(env, goal_pos_w)[:, :2],
        dim=-1,
    )
...
class success_held:
    ...
    def __call__(
        ...
        success_now = inside_goal_radius(
            env, goal_pos_w, success_radius, asset_cfg
        ) & below_speed_threshold(env, speed_threshold, asset_cfg)
    ...
    def reset(self, env_ids: torch.Tensor | slice | None = None) -> None:
        self.held_time[slice(None) if env_ids is None else env_ids] = 0.0
